...
const getItem = (z, bundle) => {
  ...
  // Do a _catch_hook() from scripting.
  const catchHookEvent = {
    name: 'trigger.hook',
    key: 'hook_trigger',
  };
  return legacyScriptingRunner.runEvent(catchHookEvent, z, bundle)
    .then((results) => {
...
module.exports = {
  ...
  display: {
    label: 'Hook Trigger',
    description: 'A trigger with just a _catch_hook (which also runs pre_subscribe, post_subscribe, and pre_unsubscribe).',
    important: true,
  },
